...
public class World
{
	...
	{
		Terrain newChunk = new Terrain(noise, seed, pos.x, pos.y, pos.z);
	...
	public void updateVisibleChunks(Vector3f viewPosition)
	{
		for (int x = -VIEW_DISTANCE; x <= VIEW_DISTANCE; x++)
		{
			for (int y = 0; y < 2; y++)
			{
				...
				{
					Pos3 chunk = new Pos3(x * (Terrain.SIZE - 1), y * (Terrain.SIZE - 1), z * (Terrain.SIZE - 1));
					getChunkFromWorldCoords(chunk.x + viewPosition.x, chunk.y + viewPosition.y, chunk.z + viewPosition.z);
				}
			}
		}
	}
